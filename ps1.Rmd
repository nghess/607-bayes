...
n_tosses = 5
sim_iters = 1000
# Empty vector to store predicted "W" counts
predicted_counts = rep(0, n_tosses)

# 1. Sample from your posterior distribution
post_samples <- sample(posterior, size=sim_iters, prob=posterior, replace=TRUE )
...
barplot(predicted_counts, names.arg = seq(1,5,1),
...
```{r}
n_tosses <- 5
# Assuming 'posterior' is a vector of probabilities that sums to 1
# And that you want to sample from the values 0:1 with these probabilities

# 1. Set up the simulation
sim_iters <- 10000
possible_counts <- 0:n_tosses
predicted_counts <- numeric(length(possible_counts))

# 2. Run the simulation
for (i in 1:sim_iters) {
  # Sample one value from the posterior
  p_water <- sample(seq(0, 1, length.out=length(posterior)), 
                   size=1, prob=posterior, replace=TRUE)
  
  # Simulate new tosses with this probability
  new_tosses <- rbinom(1, size=n_tosses, prob=p_water)
  
  # Update the count for this number of successes
  predicted_counts[new_tosses + 1] <- predicted_counts[new_tosses + 1] + 1
}

# 3. Convert to probabilities
predicted_probs <- predicted_counts / sum(predicted_counts)

# 4. Create a visualization of the predictions
barplot(predicted_probs, names.arg=possible_counts,
        main="Posterior Predictive Distribution",
        xlab="Water Count",
        ylab="Probability")
